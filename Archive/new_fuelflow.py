...
def find_t_for_s(s, p, t):
    ...
    while condition_bool:
        ...
        condition_bool = abs(s_a-s)/(s_a+s) > tolerance
        if i > max_iter:
...
def apply_total_pressure(pt, ht, t1, v):
    ...
    while condition_bool:
        ...
        p1 += pt - pt_a
        t1 += (ht-ht_a)/min(h2.calc_H2_cp(t0, p0), 8e3)
        condition_bool = (
            abs(t0-t1)/(t0+t1) > tolerance
            and abs(p0-p1)/(p0+p1) > tolerance
            )
        if i > max_iter:
            raise Exception("Exceeded number of iterations")
    return t1, p1
...
def calc_t(ht, p, v, vap):
    ...
    while condition_bool:
        ...
        condition_bool = abs(h-h_a)/(h+h_a) > tolerance
        if i > max_iter:
            raise Exception("Exceeded number of iterations")
    return t1
