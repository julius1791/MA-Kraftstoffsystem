...
def reference(t_cb, qm_hpfp, Q_fohe, Q_idg, p_hpfp, eta_hpfp, p_lpfp, eta_lpfp, qm_cb, t0, p0, v = v0, tolerance = tolerance*1e-2):
    t_r1 = 0
    i = 0
    qm_t = 0.1
...
def h2pump(t_cbt, t_hxt, eta_hpfp, p_cbt, qm_cb, t0, p0, v = v0, tolerance = tolerance):
    qm_r = 0.5
    p_hpfp = p_cbt
    ...
    while condition_bool:
        ...
        P_r, _ = h2flow.compress_thermic(p_hpfp, eta_hpfp)  # to do: give that it's own efficiency value
...
def h2after(t_cbt, t_hxt, eta_hpfp, p_cbt, qm_cb, t0, p0, v = v0, tolerance = tolerance):
    qm_r = 0.5
    p_hpfp = p_cbt
    dH = get_dh(qm_cb, t_cbt, p_cbt, t0, p0, v)
    h_r = dH/qm_cb
...
def h2dual(t_cbt, t_hxt, eta_hpfp, p_cbt, qm_cb, t0, p0, v = v0, tolerance = tolerance):
    qm_r = 0.5
    qm_v = 0.02
    p_hpfp = p_cbt
    dH = get_dh(qm_cb, t_cbt, p_cbt, t0, p0, v)
    ...
    while condition_bool:
        ...
        h2flow.compress_thermic(p_hpfp, eta_hpfp)    # to do: give that it's own efficiency value
        h_r = h2flow.calc_h()
        t_cba = ff_cb.t
        p_cba = ff_cb.p
...
def h2pre(t_cbt, t_hxt, eta_hpfp, p_cbt, qm_cb, t0, p0, v = v0, tolerance = tolerance):
    qm_r = 0.07
    p_hpfp = p_cbt
    dH = get_dh(qm_cb, t_cbt, p_cbt, t0, p0, v)
    h_r = dH/qm_cb
