...
def reference(t_cb, qm_hpfp, Q_fohe, Q_idg, p_hpfp, eta_hpfp, p_lpfp, eta_lpfp, qm_cb, t0, p0, v = v0, tolerance = tolerance):
    ...
    while abs(t_cb-t_r1) > tolerance:
        ...
        ff_main.mix_flows(jetaflow.JetaFlow(qm_hpfp-qm_t-qm_cb, t_cb, ff_main.p, v))
        ff_main.heat_fixed_power(Q_fohe)
        P_hpfp, _ = ff_main.pump_hydraulic(p_hpfp, eta_hpfp)
        ff_main.sat_p()
        ff_main.split_flows(qm_cb)
        ff_main.heat_fixed_power(Q_idg)
...
def h2pump(t_cbt, t_hxt, eta_hpfp, eta_r, p_cbt, qm_cb0, t0, p0, Q_hx = 0, pcc=False, v = v0, tolerance = tolerance):
    ...
    while condition_bool:
        ...
        ff_main.reduce_pressure(ff_main.p-100e3)
